...
public class BigTree extends TreeGenerator {
   ...
   /**
        ...
        for (; k >= 0; --k) {
            ...
            if (f >= 0.0F) {
                for (int l = 0; l < i; ++l) {
                    ...
                    Vector3 blockpos = this.basePos.add(d2, (double)(k - 1), d3);
                    Vector3 blockpos1 = position.getSide(Vector3.SIDE_UP, this.leafDistanceLimit);

                    if (this.checkBlockLine(blockpos, blockpos1) == -1) {
                        int i1 = this.basePos.getX() - position.getX();
                        int j1 = this.basePos.getZ() - position.getZ();
                        double d4 = (double)position.getY() - Math.sqrt((double)(i1 * i1 + j1 * j1)) * this.branchSlope;
                        int k1 = d4 > (double)j ? j : (int)d4;
                        Vector3 blockpos2 = new Vector3(this.basePos.getX(), k1, this.basePos.getZ());
    ...
    void crosSection(Vector3 pos, float p_181631_2_, EnumFacing p_181631_3_) {
     ...
     */
    void generateLeafNode(Vector3 pos) {
        for (int i = 0; i < this.leafDistanceLimit; ++i) {
            this.crosSection(blockpos.up(i), this.leafSize(i), Blocks.LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }

    void limb(Vector3 p_175937_1_, Vector3 p_175937_2_, Block p_175937_3_) {
        Vector3 blockpos = p_175937_2_.add(-p_175937_1_.getX(), -p_175937_1_.getY(), -p_175937_1_.getZ());
        ...
        for (int j = 0; j <= i; ++j) {
            Vector3 blockpos1 = p_175937_1_.add((double)(0.5F + (float)j * f), (double)(0.5F + (float)j * f1), (double)(0.5F + (float)j * f2));
            BlockLog.EnumAxis blocklog$enumaxis = this.getLogAxis(p_175937_1_, blockpos1);
            this.setBlockAndNotifyAdequately(level, blockpos1, p_175937_3_.getDefaultState().withProperty(BlockLog.LOG_AXIS, blocklog$enumaxis));
        }
    }
     ...
     */
    void generateLeaves() {
        for (BigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
     ...
     */
    void generateTrunk() {
        Vector3 blockpos = this.basePos;
        Vector3 blockpos1 = this.basePos.up(this.height);
    ...
    void generateLeafNodeBases() {
        for (BigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
            ...
            Vector3 blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());
     ...
     */
    int checkBlockLine(Vector3 posOne, Vector3 posTwo) {
        Vector3 blockpos = posTwo.add(-posOne.getX(), -posOne.getY(), -posOne.getZ());
        int i = this.getGreatestDistance(blockpos);
        float f = (float)blockpos.getX() / (float)i;
        float f1 = (float)blockpos.getY() / (float)i;
        float f2 = (float)blockpos.getZ() / (float)i;
        ...
        if (i == 0) {
            ...
            for (int j = 0; j <= i; ++j) {
                Vector3 blockpos1 = posOne.add((double)(0.5F + (float)j * f), (double)(0.5F + (float)j * f1), (double)(0.5F + (float)j * f2));
    ...
    public boolean generate(ChunkManager worldIn, Random rand, Vector3 position) {
        this.world = worldIn;
    ...
    static class FoliageCoordinates extends Vector3 {
        private final int branchBase;

        public FoliageCoordinates(Vector3 pos, int p_i45635_2_) {
